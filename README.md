...
- LDR (Light ___ Resistor)
- ____ (Soil Moisture sensor)

All of this would go through an ESP32-S2/S3 and sent via MQTT protocols (Topic and Subscribe) to a Database (_insert name of DB_).
These sensor values would be compared with data from a public Plant API to determine if these values are too high or too low.

A Dashboard in the form of a Web App or Flutter App would show your Plant, its current values, the ideal values, and what should be done right now.
